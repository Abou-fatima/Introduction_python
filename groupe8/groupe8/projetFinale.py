# -*- coding: utf-8 -*-
"""
Created on Wed May 21 11:57:22 2025
...
def creer_tables():
    ...
    curseur.close()
    conn.close()
def lancer_authentification():
    creer_tables()
    global fenetre, EntryMailUser, EntryPassWordUser

    fenetre = tk.Tk()
    fenetre.geometry("500x300+400+200")
    fenetre.title("AUTHENTIFICATION")
    fenetre['background'] = "blue"
    fenetre.resizable(False, False)

    labelLogin = tk.Label(fenetre, text="AUTHENTIFICATION", width=20, fg="red", font=("Arial",25))
    labelLogin.place(x=70, y=20)

    labelMailUser = tk.Label(fenetre, text="E-Mail", width=15, fg="red", font=("Arial",12))
    labelMailUser.place(x=15,y=90)

    EntryMailUser = tk.Entry(fenetre)
    EntryMailUser.place(x=165, y=90, width=200, height=25)

    labelPassWordUser = tk.Label(fenetre, text="Mot de passe", width=15, fg="red", font=("Arial",12))
    labelPassWordUser.place(x=15,y=150)

    EntryPassWordUser = tk.Entry(fenetre, show="*")
    EntryPassWordUser.place(x=165, y=150, width=200, height=25)

    BouttonConnection = tk.Button(fenetre, text="Se connecter", width=15, fg="blue", font=("Arial",10),command=se_connecter)
    BouttonConnection.place(x=230, y=200)

    fenetre.mainloop()

def se_connecter():
    email = EntryMailUser.get().strip()
    mot_de_passe = EntryPassWordUser.get().strip()
    ...
    try:
        conn = connecter()
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM users WHERE email = %s AND mot_de_passe = %s", (email, mot_de_passe_hash))
        utilisateur = cursor.fetchone()
        conn.close()

        if utilisateur:
            progress = ttk.Progressbar(fenetre, orient="horizontal", length=550, mode="determinate")
            progress.place(x=0, y=250)
            progress["maximum"] = 150
            for i in range(151):
                progress["value"] = i
                fenetre.update()
                fenetre.after(10)  # Vitesse de la progression (10 ms)
            progress.destroy()
            fenetre.destroy()   # Fermer la fenêtre de connexion   # Fermer la fenêtre de connexion
            lancer_principal()
        else:
...
def deconnexion():
    root.destroy()
    lancer_authentification()

def lancer_principal():
    global root
    root = tk.Tk()
    ...
    image = Image.open("C:/Users/abouf/.spyder-py3/groupe8/image3.jpg")  # Mets le chemin de ton image ici
    image = image.resize((650, 500))  # Adapter à la taille de la fenêtre
    ...
    def ouvrir_fenetre_enregistrer_eleve():
        ...
        def exporter_en_excel():
            chemin_fichier = "C:/Users/abouf/.spyder-py3/groupe8/liste_eleves.xlsx"
            try:
                conn = connecter()
                cursor = conn.cursor()
        ...
        def exporter_vers_pdf():
            ...
            try:
                ...
                chemin_fichier = 'C:/Users/abouf/.spyder-py3/groupe8/liste_eleves.pdf'
        ...
        def ajouter_filigrane(canvas, doc):
            # Charger le logo
            try:
                logo_path = 'C:/Users/abouf/.spyder-py3/groupe8/logoIst.jpg'  # Mets ici ton vrai chemin
    ...
    def ouvrir_fenetre_filtrage():
        ...
        df = pd.read_excel("C:/Users/abouf/.spyder-py3/groupe8/liste_eleves.xlsx")
        df_filtré = df.copy()
        ...
        def exporter_excel():
            if df_filtré.empty:
                ...
                return
            chemin = 'C:/Users/abouf/.spyder-py3/groupe8/eleves_filtres.xlsx'
            df_filtré.to_excel(chemin, index=False)
            messagebox.showinfo("Excel", f"Données filtrées exportées vers : {chemin}")
            
        def exporter_pdf():
                if df_filtré.empty:
                    messagebox.showwarning("PDF", "Aucune donnée filtrée à exporter.")
                    return
                from reportlab.platypus import SimpleDocTemplate, Table, TableStyle, Paragraph, Spacer
                from reportlab.lib.pagesizes import letter
                from reportlab.lib import colors
                ...
                chemin = 'C:/Users/abouf/.spyder-py3/groupe8/eleves_filtres.pdf'
                doc = SimpleDocTemplate(chemin, pagesize=letter)
                styles = getSampleStyleSheet()
                elements = []
            
                titre = Paragraph("Liste filtrée des élèves", styles['Title'])
                elements.append(titre)
                elements.append(Spacer(1, 12))
            
                # Préparer les données pour le tableau
                data = [list(df_filtré.columns)] + df_filtré.values.tolist()
                table = Table(data)
                style = TableStyle([
                    ('BACKGROUND', (0, 0), (-1, 0), colors.lightblue),
                    ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
                ...
                try:
                    doc.build(elements)
                    messagebox.showinfo("PDF", f"Données filtrées exportées vers : {chemin}")
                except Exception as e:
                    messagebox.showerror("Erreur", f"Erreur lors de l'export PDF : {e}")
        ...
        BouttonExportPdf = tk.Button(fenetre, text="Exportez en pdf", width=15, command=exporter_pdf)
...
#    menubar.add_cascade(label="Gestion des Paiements", menu=menu_gestion_paiement)
    ...
    menu_gestion_parametre.add_command(label="Deconnexion", command=deconnexion)
